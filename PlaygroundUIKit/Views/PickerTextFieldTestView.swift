...
class PickerTextFieldTestView: UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func viewLoad() {
        ...
        NSLayoutConstraint.activate([
            // 入力欄
            input.widthAnchor.constraint(equalTo: widthAnchor, multiplier: 0.9),
            input.centerXAnchor.constraint(equalTo: centerXAnchor),
            input.centerYAnchor.constraint(equalTo: centerYAnchor, constant: -100),
            // 入力用TextField
            inputTextField.topAnchor.constraint(equalTo: input.topAnchor, constant: 12),
            inputTextField.bottomAnchor.constraint(equalTo: input.bottomAnchor, constant: -12),
            inputTextField.leadingAnchor.constraint(equalTo: input.leadingAnchor, constant: 12),
            inputTextField.trailingAnchor.constraint(equalTo: input.trailingAnchor, constant: -12),
            // 入力用TextFieldの横に配置する下矢印マーク
            underArrowImageView.topAnchor.constraint(equalTo: inputTextField.topAnchor),
            underArrowImageView.bottomAnchor.constraint(equalTo: inputTextField.bottomAnchor),
            underArrowImageView.trailingAnchor.constraint(equalTo: inputTextField.trailingAnchor)
        ])
    }
